...
if [ -z "$1" ]; then
    ...
    echo ""
    echo -e "${YELLOW}🔑 HANGİ API KEY GEREKİYOR?${NC}"
    echo -e "${RED}   ❌ Claude'un API key'i DEĞİL!${NC}"
    echo -e "${GREEN}   ✅ OpenMemory API Key gerekiyor${NC}"
    echo ""
    echo -e "${YELLOW}📍 Nereden Alınır?${NC}"
    echo "   1. https://openmemory.ai sitesine git"
    echo "   2. Sign Up (ücretsiz hesap)"
    echo "   3. Dashboard'a gir"
    echo "   4. API Keys → Create New Key"
    echo "   5. Key'i kopyala (başında om_ var)"
    echo ""
    echo -e "${YELLOW}📝 Key Formatı:${NC}"
    echo "   om_abc123xyz789..."
    echo ""
    echo -e "${YELLOW}💡 OpenMemory Nedir?${NC}"
    echo "   Claude Desktop için akıllı hafıza servisi:"
    echo "   ✅ Konuşmalarını hatırlar"
    echo "   ✅ Proje bağlamını saklar"
    echo "   ✅ Otomatik öneriler"
    echo "   ✅ Daha akıllı yanıtlar"
    echo ""
    echo -e "${GREEN}🆓 Ücretsiz mi? Evet! Free tier mevcut.${NC}"
    echo ""
