...
export async function generateStaticParams() {
  return baglantiData.bolumler.map((bolum) => ({ bolum: bolum.slug }));
}

export async function generateMetadata({ params }: { params: { bolum: string } }): Promise<Metadata> {
  const bolum = baglantiData.bolumler.find((b) => b.slug === params.bolum);

  if (!bolum) {
    return {
      title: 'Bölüm Bulunamadı | OLUK BAĞLANTI',
      description: 'İstenen bölüm sayfası bulunamadı.'
    };
  }

  return {
    title: `${bolum.title}: ${bolum.subtitle} - Ders | OLUK BAĞLANTI`,
...
export default function BolumDersPage({ params }: { params: { bolum: string } }) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/moduller/baglanti" className="hover:underline" style={{ color: accent }}>
            BAĞLANTI Modülüne Dön
          </Link>
        </div>
      </div>
    );
  ...
  const prevBolum = baglantiData.bolumler.find((b) => b.order === bolum.order - 1);
  const nextBolum = baglantiData.bolumler.find((b) => b.order === bolum.order + 1);
    ...
    <main className="min-h-screen bg-cream">
      ...
      <div className="px-6 py-4" style={{ backgroundColor: `${primary}0D` }}>
        <div className="max-w-4xl mx-auto flex items-center gap-2 text-sm" style={{ color: `${primary}99` }}>
          <Link href="/moduller" className="hover:text-[#2D6A4F] transition">Modüller</Link>
          <span>/</span>
          <Link href="/moduller/baglanti" className="hover:text-[#2D6A4F] transition">BAĞLANTI</Link>
          <span>/</span>
      ...
      <header className="text-cream py-12 px-6" style={{ background: `linear-gradient(180deg, ${primary}, ${primary}E6)` }}>
        <div className="max-w-4xl mx-auto text-center">
          <Link href="/moduller/baglanti" className="inline-flex items-center gap-2 text-white/70 hover:text-white transition mb-6">
            <ChevronLeft className="w-4 h-4" />
            <span>BAĞLANTI Modülüne Dön</span>
          </Link>
      ...
      <section className="py-16 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-[#F7FBF8] rounded-2xl p-8 mb-12 border" style={{ borderColor: `${accent}33` }}>
          ...
          <div className="bg-cream rounded-lg p-6 mb-12 border-l-4" style={{ borderColor: accent }}>
            ...
            <Link href={`/moduller/baglanti/${bolum.slug}`} className="inline-flex items-center gap-2 font-semibold hover:gap-3 transition" style={{ color: accent }}>
          ...
          <div className="flex gap-4 pt-8 border-t" style={{ borderColor: `${accent}33` }}>
            {prevBolum ? (
              <Link href={`/moduller/baglanti/${prevBolum.slug}/ders`} className="flex-1">
            ...
            <Link href={`/moduller/baglanti/${bolum.slug}`}>
              <div className="bg-cream rounded-lg p-4 hover:bg-cream/80 transition text-center">
            ...
            {nextBolum ? (
              <Link href={`/moduller/baglanti/${nextBolum.slug}/ders`} className="flex-1">
                <div className="bg-cream rounded-lg p-4 hover:bg-cream/80 transition text-center">
