...
export async function generateStaticParams() {
  ...
  baglantiData.bolumler.forEach((bolum) => {
...
export async function generateMetadata({ params }: { params: { bolum: string; seansNo: string } }): Promise<Metadata> {
  ...
  if (!bolum || !seans) {
    return {
      title: 'Seans Bulunamadı | OLUK BAĞLANTI',
      description: 'İstenen seans bulunamadı.'
    };
  }

  return {
    title: `${seans.title} - ${bolum.title} | OLUK BAĞLANTI`,
...
export default function SeansPage({ params }: { params: { bolum: string; seansNo: string } }) {
  ...
  const nextBolum = nextSeans ? null : baglantiData.bolumler.find((b) => b.order === bolum.order + 1);
    ...
    <main className="min-h-screen bg-cream">
      ...
      <div className="px-6 py-4" style={{ backgroundColor: `${primary}0D` }}>
        <div className="max-w-4xl mx-auto flex items-center gap-2 text-sm" style={{ color: `${primary}99` }}>
          ...
          <span>/</span>
          <Link href={`/moduller/baglanti/${bolum.slug}`} className="hover:text-[#2D6A4F] transition">
            {bolum.title}
          </Link>
          <span>/</span>
      ...
      <header className="text-cream py-12 px-6" style={{ background: `linear-gradient(180deg, ${primary}, ${primary}E6)` }}>
        <div className="max-w-4xl mx-auto text-center">
          <Link href={`/moduller/baglanti/${bolum.slug}`} className="inline-flex items-center gap-2 text-white/70 hover:text-white transition mb-6">
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-2xl p-8 shadow-sm border" style={{ borderColor: `${accent}33` }}>
            ...
            <div className="flex gap-4 pt-8 border-t" style={{ borderColor: `${accent}33` }}>
              {prevSeans ? (
                <Link href={`/moduller/baglanti/${bolum.slug}/seans-${seansNo - 1}`} className="flex-1">
                ...
                <Link href={`/moduller/baglanti/${bolum.slug}/ders`} className="flex-1">
                  <div className="bg-cream rounded-lg p-4 hover:bg-cream/80 transition text-center">
