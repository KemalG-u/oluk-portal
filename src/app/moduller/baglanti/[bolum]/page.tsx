// BAĞLANTI Bölüm Index Sayfası
// /moduller/baglanti/[bolum] - ders ve seanslar
...
export async function generateStaticParams() {
  return baglantiData.bolumler.map((bolum) => ({
    bolum: bolum.slug,
  }));
}
...
export async function generateMetadata({ params }: { params: { bolum: string } }): Promise<Metadata> {
  ...
  return {
    title: `${bolum.title}: ${bolum.subtitle} | OLUK BAĞLANTI`,
      ...
      <section className="py-16 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-16">
            ...
            <Link href={`/moduller/baglanti/${bolum.slug}/ders`}>
              <div className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all overflow-hidden cursor-pointer group border-l-4" style={{ borderColor: accent }}>
                <div className="p-8 flex items-start justify-between" style={{ background: `${accent}14` }}>
                  <div className="flex-1">
                    ...
                    <h3 className="font-serif text-2xl mb-2 group-hover:text-[#2D6A4F] transition" style={{ color: primary }}>
                      {bolum.ders.title}
                    </h3>
          ...
          <div>
            ...
            <div className="space-y-4">
              {bolum.seans.map((seans, idx) => (
                <Link key={seans.id} href={`/moduller/baglanti/${bolum.slug}/seans-${idx + 1}`}>
                  <div className="bg-white rounded-xl shadow-md hover:shadow-lg transition-all border-l-4 p-6 cursor-pointer group" style={{ borderColor: accent }}>
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <h3 className="font-serif text-xl mb-2 group-hover:text-[#2D6A4F] transition" style={{ color: primary }}>
                          {seans.title}
                        </h3>
          ...
          <div className="flex gap-4 mt-16 pt-8 border-t" style={{ borderColor: `${accent}33` }}>
            {prevBolum ? (
              <Link href={`/moduller/baglanti/${prevBolum.slug}`} className="flex-1">
            ...
            {nextBolum ? (
              <Link href={`/moduller/baglanti/${nextBolum.slug}`} className="flex-1">
                <div className="bg-cream rounded-lg p-6 hover:bg-cream/80 transition text-center">
